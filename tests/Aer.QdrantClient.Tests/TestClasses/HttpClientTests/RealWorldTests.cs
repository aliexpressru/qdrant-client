...
internal class RealWorldTests : QdrantTestsBase
{
    ...
    [Test]
    public async Task RestoreReplicationFactor()
    {
        var rrfResult = await _qdrantHttpClient.RestoreShardReplicationFactor("recovery_test_2", CancellationToken.None);

        rrfResult.Should().NotBeNull();

        rrfResult.Result.ShardsNeedReplication.Should().BeTrue();
        rrfResult.Result.ReplicationPlan.Should().NotBeEmpty();
    }
