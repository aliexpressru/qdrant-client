...
/// </summary>
internal sealed class DisableActivityHttpClientHandler(HttpMessageHandler innerHandler) : DelegatingHandler(innerHandler)
{
...
internal sealed class ConditionalPropagator : DistributedContextPropagator
{
    ...
    public override void ExtractTraceIdAndState(
        ...
        out string traceId,
        out string traceState) => _originalPropagator.ExtractTraceIdAndState(carrier, getter, out traceId, out traceState);

    public override IEnumerable<KeyValuePair<string, string>> ExtractBaggage(
        object carrier,
        PropagatorGetterCallback getter) => _originalPropagator.ExtractBaggage(carrier, getter);
}
